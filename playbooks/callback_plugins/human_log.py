...
class CallbackModule(object):
    ...
    def human_log(self, data):
        if type(data) == dict:
            ...
            if self.DEBUG:
                print("available keys: " + repr(data.keys()))
            for field in FIELDS:
                no_log = data.get('_ansible_no_log')
                # print("*** checking field: {0} ".format(field))

                if field in data.keys() and data[field] and no_log is not True:
                    # if self.DEBUG:
                    #     print("\nformatting field: %s" % field)
    ...
    def runner_on_failed(self, host, res, ignore_errors=False):
        if self.DEBUG:
            print("called: runner_on_failed")
        self.human_log(res)

    def runner_on_ok(self, host, res):
        if self.DEBUG:
            print("called: runner_on_ok")
        self.human_log(res)
    ...
    def runner_on_unreachable(self, host, res):
        if self.DEBUG:
            print("called: runner_on_unreachable")
        self.human_log(res)
    ...
    def runner_on_async_poll(self, host, res, jid, clock):
        if self.DEBUG:
            print("called: runner_on_async_poll")
        self.human_log(res)

    def runner_on_async_ok(self, host, res, jid):
        if self.DEBUG:
            print("called: runner_on_async_ok")
        self.human_log(res)

    def runner_on_async_failed(self, host, res, jid):
        if self.DEBUG:
            print("called: runner_on_async_failed")
        self.human_log(res)
    ...
    def v2_on_any(self, *args, **kwargs):
        # if self.DEBUG:
        #     print("called: v2_on_any")
        pass

    def v2_runner_on_failed(self, result, ignore_errors=False):
        if self.DEBUG:
            print("called: v2_runner_on_failed")
    ...
    def v2_runner_on_unreachable(self, result):
        if self.DEBUG:
            print("called: v2_runner_on_unreachable")
        self.human_log(result._result)
    ...
    def v2_runner_on_async_poll(self, result):
        if self.DEBUG:
            print("called: v2_runner_on_async_poll")
        self.human_log(result._result)

    def v2_runner_on_async_ok(self, host, result):
        if self.DEBUG:
            print("called: v2_runner_on_async_ok")
        self.human_log(result._result)

    def v2_runner_on_async_failed(self, result):
        if self.DEBUG:
            print("called: v2_runner_on_async_failed")
        self.human_log(result._result)
    ...
    def v2_playbook_on_include(self, included_file):
        if self.DEBUG:
            print("called: v2_playbook_on_include")
        pass

    def v2_runner_item_on_ok(self, result):
        if self.DEBUG:
            print("called: v2_runner_item_on_ok")
        self.human_log(result._result)

    def v2_runner_item_on_failed(self, result):
        if self.DEBUG:
            print("called: v2_runner_item_on_failed")
        self.human_log(result._result)

    def v2_runner_item_on_skipped(self, result):
        if self.DEBUG:
            print("called: v2_runner_item_on_skipped")
        self.human_log(result._result)
